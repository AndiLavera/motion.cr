  ...
  describe Motion::Channel do
    describe "with #{adapter} adapter" do
      ...
      it "can handle a new subscriber" do
        ...
        channel.connection_manager.adapter.get_component(MESSAGE_JOIN["topic"].as_s).should_not be_nil
        channel.connection_manager.adapter.get_component(MESSAGE_JOIN["topic"].as_s).class.should eq(MotionRender)
      end
      ...
      it "raises NoComponentConnectionError on mismatch topic" do
        ...
        expect_raises(Motion::Exceptions::NoComponentConnectionError) do
          channel.connection_manager.adapter.get_component("bad_topic")
        end
      end
      ...
      it "can process a motion" do
        ...
        channel.connection_manager.adapter.get_component(MESSAGE_JOIN["topic"].as_s).view.to_s.empty?.should be_true

        channel.handle_message(nil, MESSAGE_NEW)
        component = channel.connection_manager.adapter.get_component(MESSAGE_JOIN["topic"].as_s)
      ...
      it "can process broadcast streams" do
        ...
        component = channel.connection_manager.adapter.get_component("motion:69689")
