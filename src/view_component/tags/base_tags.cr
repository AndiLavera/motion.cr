module ViewComponent::BaseTags
  include ViewComponent::CheckTagContent
  ...
  macro generate_tag_methods(method_name, tag)
    ...
    #
    # * The *content* argument is either a `String`, or any type that has included `ViewComponent::AllowedInTags`. This is the content that goes inside of the tag.
    ...
    # ```
    def {{method_name.id}}(
        content : ViewComponent::AllowedInTags | String = "",
    ...
    def {{method_name.id}}(content : String | ViewComponent::AllowedInTags) : Nil
  ...
  # text("<div>") # => &lt;div&gt;
  # ```
  def text(content : String | ViewComponent::AllowedInTags) : Nil
  ...
  private def merge_options(html_options, tag_attrs)
    options = {} of String => String | ViewComponent::AllowedInTags
